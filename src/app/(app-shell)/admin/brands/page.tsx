...
const BrandList = () => {
  ...
  const fetchBrands = async () => {
    try {
      ...
      setError(t('admin.brands_fetch_failed') || 'Failed to fetch brands.');
  ...
  const handleDelete = async (id: string) => {
    ...
    setConfirmDialogMessage(t('admin.brands.delete_confirm') || 'Are you sure you want to delete this brand?');
    setConfirmDialogAction(() => async () => {
      try {
        await deleteBrand(id);
        setBrands(brands.filter(brand => brand.id !== id));
        setSelectedIds(new Set());
        setInfoDialogMessage(t('admin.brands_delete_success') || 'Brand deleted successfully!');
        setInfoDialogType('success');
        setShowInfoDialog(true);
      } catch {
        setInfoDialogMessage(t('admin.brands.delete_failed') || 'Failed to delete brand.');
        setInfoDialogType('error');
        setShowInfoDialog(true);
      }
    });
  ...
  const handleBatchDelete = async () => {
    ...
    setConfirmDialogAction(() => async () => {
      try {
        ...
        setInfoDialogMessage(t('admin.brands_batch_delete_failed') || 'Failed to delete brands.');
        setInfoDialogType('error');
        setShowInfoDialog(true);
      }
    });
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-semibold text-gray-900">{t('admin.brands') || 'Brands'}</h1>
          <p className="text-gray-500 text-sm mt-1">{t('admin.brands_subtitle') || 'Manage product brands'}</p>
        </div>
        <Link
        ...
        >
          ...
          {t('admin.brands_add_new') || 'Add Brand'}
        </Link>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        {brands.length === 0 ? (
          <div className="p-8 sm:p-12 text-center text-gray-500">
            <p className="text-base sm:text-lg font-medium mb-2">{t('admin.common.no_items_found') || 'No items found'}</p>
            <p className="text-sm text-gray-400">{t('admin.brands_empty_message') || 'Get started by creating your first brand'}</p>
          </div>
        ) : (
          <>
            {/* Desktop Table View */}
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th scope="col" className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">{t('admin.brands_table_name') || 'Name'}</th>
                    <th scope="col" className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">{t('admin.brands_table_slug') || 'Slug'}</th>
                    <th scope="col" className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">{t('admin.brands_table_description') || 'Description'}</th>
                    <th scope="col" className="px-4 sm:px-6 py-3 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider">{t('admin.brands_table_actions') || 'Actions'}</th>
                  </tr>
                </thead>
            ...
            {totalPages > 1 && (
              <div className="bg-gray-50 px-4 sm:px-6 py-3 border-t border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div className="text-sm text-gray-700">
                  {t('admin.common.showing') || 'Showing'} {startIndex + 1} {t('admin.common.to') || 'to'} {Math.min(endIndex, brands.length)} {t('admin.common.of') || 'of'} {brands.length} {t('admin.common.results') || 'results'}
                </div>
