...
const ShippingCarriersPage = () => {
  ...
  const handleDelete = async (id: string) => {
    ...
    setConfirmDialogMessage(t('admin.shipping_carriers_delete_confirm') || 'Are you sure you want to delete this carrier?');
    setConfirmDialogAction(() => async () => {
      try {
        await deleteShippingCarrier(id);
        setCarriers(carriers.filter((c) => c.id !== id));
        setSelectedIds(new Set());
        setInfoDialogMessage(t('admin.shipping_carriers_delete_success') || 'Shipping carrier deleted successfully!');
        setInfoDialogType('success');
        setShowInfoDialog(true);
      } catch {
        // Failed to delete carrier
        setInfoDialogMessage(t('admin.shipping_carriers_delete_failed') || 'Failed to delete carrier.');
        setInfoDialogType('error');
        setShowInfoDialog(true);
      }
    });
  ...
  const handleBatchDelete = async () => {
    ...
    setConfirmDialogAction(() => async () => {
      try {
        ...
        setInfoDialogMessage(t('admin.shipping_carriers_batch_delete_failed') || 'Failed to delete carriers.');
        setInfoDialogType('error');
        setShowInfoDialog(true);
      }
    });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-semibold text-gray-900">
            {t('admin.shipping_carriers_title') || 'Shipping Carriers'}
          </h1>
          <p className="text-gray-500 text-sm mt-1">
            {t('admin.shipping_carriers_subtitle') ||
              'Manage shipping carriers and methods'}
          </p>
        </div>
        <div className="flex gap-2 sm:gap-3 flex-wrap">
          ...
          >
            <option value="all">
              {t('admin.shipping_carriers_filter_all') || 'All Carriers'}
            </option>
            <option value="active">
              {t('admin.shipping_carriers_filter_active') || 'Active Only'}
            </option>
            <option value="inactive">
              {t('admin.shipping_carriers_filter_inactive') || 'Inactive Only'}
            </option>
          </select>
          <Link
          ...
          >
            ...
            {t('admin.shipping_carriers_add_button') || 'Add Carrier'}
          </Link>
        </div>
      </div>
        ...
        {carriers.length === 0 ? (
          <div className="p-12 text-center text-gray-500">
            ...
            <p className="text-lg font-medium">
              {t('admin.shipping_carriers_empty_title') || 'No carriers found'}
            </p>
                ...
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                      {t('admin.shipping_carriers_table_name') || 'Name'}
                    </th>
                    <th className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                      {t('admin.shipping_carriers_table_code') || 'Code'}
                    </th>
                    <th className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                      {t('admin.shipping_carriers_table_contact') || 'Contact'}
                    </th>
                    <th className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                      {t('admin.shipping_carriers_table_tracking_url') || 'Tracking URL'}
                    </th>
                    <th className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                      {t('admin.shipping_carriers_table_status') || 'Status'}
                    </th>
                    <th className="px-4 sm:px-6 py-3 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider">
                      {t('admin.shipping_carriers_table_actions') || 'Actions'}
                    </th>
                  </tr>
                </thead>
                    ...
                    <tr key={carrier.id} className={`hover:bg-gray-50 transition-colors ${selectedIds.has(carrier.id!) ? 'bg-blue-50' : ''}`}>
                      ...
                      <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                        <span className={`px-2.5 py-1 inline-flex text-xs font-semibold rounded-md ${
                          ...
                          {carrier.isActive
                            ? t('admin.shipping_carriers_status_active') || 'Active'
                            : t('admin.shipping_carriers_status_inactive') || 'Inactive'}
                        </span>
                      </td>
                  ...
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="space-y-1 text-xs text-gray-600">
                        <p><span className="font-medium">{t('admin.shipping_carriers_table_code') || 'Code'}:</span> <span className="font-mono">{carrier.code}</span></p>
                        <p><span className="font-medium">{t('admin.shipping_carriers_table_contact') || 'Phone'}:</span> {carrier.phone || t('admin.shipping_carriers_contact_phone_placeholder') || '-'}</p>
                        {carrier.email && (
                          <p><span className="font-medium">{t('admin.shipping_carriers_table_email') || 'Email'}:</span> {carrier.email}</p>
                        )}
                        {carrier.trackingUrl && (
                          <p><span className="font-medium">{t('admin.shipping_carriers_table_tracking_url') || 'Tracking'}:</span> <a href={carrier.trackingUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline truncate block">{carrier.trackingUrl}</a></p>
                        )}
                      </div>
                    </div>
            ...
            {totalPages > 1 && (
              <div className="bg-gray-50 px-4 sm:px-6 py-3 border-t border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div className="text-sm text-gray-700">
                  {t('admin.common.showing') || 'Showing'} {startIndex + 1} {t('admin.common.to') || 'to'} {Math.min(endIndex, carriers.length)} {t('admin.common.of') || 'of'} {carriers.length} {t('admin.common.results') || 'results'}
                </div>
